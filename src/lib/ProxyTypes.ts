...
type QueryableFieldsFor<T> = T extends number
  ? Exclude<keyof T, keyof number>
  : T extends string
    ? Exclude<keyof T, keyof string>
    : T extends Date
      ? Exclude<keyof T, keyof Date>
      : T extends Array<unknown>
        ? Exclude<keyof T, keyof Array<unknown>>
        : T extends object
          ? Exclude<keyof T, keyof object>
...
export type EntityProxy<T, IncludeFilterMethods = false> = {
  [P in QueryableFieldsFor<T>]: PropertyProxy<T[P], IncludeFilterMethods> &
    (IncludeFilterMethods extends true ? ProxyFilterMethods<T[P]> : object);
} & {
