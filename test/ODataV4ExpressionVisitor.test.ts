...
describe("ODataV4ExpressionVisitor", () => {
  it("should handle single select", () => {
    ...
    const expression = new Expression(ExpressionOperator.Select, [
      new FieldReference<Person>("firstName"),
      new FieldReference<Person>("age"),
    ]);
  ...
  it("should handle compound orderByDescending", () => {
    ...
    visitor.visit(
      new Expression(ExpressionOperator.OrderByDescending, [new FieldReference<Person>("age")], expression),
    );

    expect(visitor.oDataQuery).to.eql({
      orderBy: [
  ...
  it("should handle getByKey with guid", () => {
    const visitor = new ODataV4ExpressionVisitor();
    visitor.visit(
      new Expression(ExpressionOperator.GetByKey, [
        Expression.literal("00000000-0000-0000-000000000000", ODataType.Guid),
      ]),
    );
  ...
  it("should handle expandAll then expand", () => {
    ...
    visitor.visit(
      new Expression(
        ...
        [new FieldReference<Person>("children")],
        new Expression(ExpressionOperator.ExpandAll, []),
      ),
    );
  ...
  it("should handle expand then expandAll", () => {
    ...
    visitor.visit(
      new Expression(
        ExpressionOperator.ExpandAll,
        [],
        new Expression(ExpressionOperator.Expand, [new FieldReference<Person>("children")]),
      ),
    );
