...
describe("requestInit", () => {
    ...
    it("should use provided object in fetch", async () => {
        ...
        const query = ODataV4QueryProvider.createQuery<Subject>(endpoint, { requestInit: () => requestInit }).filter(p => p.Name.$contains("vet"));
        await query.getManyAsync();
    ...
    it("should use provided object via Promise in fetch", async () => {
        const requestInit: RequestInit = { method: 'POST' };
        const query = ODataV4QueryProvider.createQuery<Subject>(endpoint, { requestInit: () => Promise.resolve(requestInit) }).filter(p => p.Name.$contains("vet"));
        await query.getManyAsync();
        expect(currentFetch.lastInit).to.equal(requestInit);
    });
});